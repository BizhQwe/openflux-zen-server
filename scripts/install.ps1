...
if (-not $isAdmin) {
    ...
    if ($MyInvocation.MyCommand.Path) {
        ...
        Start-Process powershell -Verb RunAs -ArgumentList "-NoProfile -ExecutionPolicy Bypass -Command `"iwr -useb https://raw.githubusercontent.com/BizhQwe/openflux-zen-server/main/scripts/install.ps1 | iex`""
    }
    exit
}
...
if (-not $chosenLang) {
    ...
    Write-Host "==================================================================" -ForegroundColor Cyan
    Write-Host "  1) English (web panel & installer)"
    Write-Host "  2) Russian / Русский (панель управления и установщик)"
    Write-Host "------------------------------------------------------------------" -ForegroundColor Cyan
    $langInput = Read-Host "Select language [1/2, default: 1]"
    $chosenLang = if ($langInput -match "2|ru") { "ru" } else { "en" }
}

# 3. Check or Install .NET 10 SDK
Write-Host ""
$txtStep1 = if ($chosenLang -eq "ru") { "[1/7] Проверка и подготовка среды .NET 10..." } else { "[1/7] Checking and preparing .NET 10 environment..." }
Write-Host $txtStep1 -ForegroundColor Cyan
...
if (-not $hasDotnet10 -and (Test-Path "$env:ProgramFiles\dotnet\dotnet.exe")) {
    $env:PATH = "$env:ProgramFiles\dotnet;$env:PATH"
    try {
        $sdks = & "$env:ProgramFiles\dotnet\dotnet.exe" --list-sdks 2>$null
        if ($sdks -match "^10\.") {
            $hasDotnet10 = $true
            Write-Host "✓ .NET 10 SDK found in $env:ProgramFiles\dotnet" -ForegroundColor Green
        }
    } catch {}
}

if (-not $hasDotnet10) {
    $txtDotnetInstall = if ($chosenLang -eq "ru") { "Установка .NET 10 SDK через официальный установщик Microsoft..." } else { "Installing .NET 10 SDK via official Microsoft installer..." }
    Write-Host $txtDotnetInstall -ForegroundColor Yellow
    [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
    $dotnetInstallerScript = "$env:TEMP\dotnet-install.ps1"
    Invoke-WebRequest -Uri "https://dot.net/v1/dotnet-install.ps1" -OutFile $dotnetInstallerScript -UseBasicParsing
    & $dotnetInstallerScript -Channel 10.0 -InstallDir "$env:ProgramFiles\dotnet"
    $env:DOTNET_ROOT = "$env:ProgramFiles\dotnet"
    $env:PATH = "$env:ProgramFiles\dotnet;$env:PATH"
    
    $currMachinePath = [Environment]::GetEnvironmentVariable("Path", "Machine")
    if ($currMachinePath -notmatch [regex]::Escape("$env:ProgramFiles\dotnet")) {
        [Environment]::SetEnvironmentVariable("Path", "$env:ProgramFiles\dotnet;$currMachinePath", "Machine")
    }
...
$txtStep2 = if ($chosenLang -eq "ru") { "[2/7] Получение исходного кода OpenFlux Zen Server..." } else { "[2/7] Fetching OpenFlux Zen Server repository..." }
Write-Host $txtStep2 -ForegroundColor Cyan

$workDir = "$env:TEMP\openflux-zen-server-build"
...
if ($scriptDir -and (Test-Path "$scriptDir\..\src\OpenFlux.Zen.Server.Web\OpenFlux.Zen.Server.Web.csproj")) {
    Write-Host "Using local repository files from $scriptDir\.." -ForegroundColor Gray
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        & git clone --depth 1 "https://github.com/BizhQwe/openflux-zen-server.git" $workDir | Out-Null
    } else {
        Write-Host "Git not found. Downloading repository zip archive from GitHub..." -ForegroundColor Gray
        $zipPath = "$env:TEMP\openflux-zen-server.zip"
        [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
        ...
        $extractDir = "$env:TEMP\openflux_extracted"
...
# 5. Build & Publish Web & CLI
$txtStep3 = if ($chosenLang -eq "ru") { "[3/7] Сборка и публикация компонентов (Web и CLI)..." } else { "[3/7] Building and publishing application components (Web & CLI)..." }
Write-Host $txtStep3 -ForegroundColor Cyan

$InstallDir = "$env:ProgramFiles\OpenFluxZenServer"
...
& dotnet publish "$workDir\src\OpenFlux.Zen.Server.Web\OpenFlux.Zen.Server.Web.csproj" -c Release -o $InstallDir | Out-Null
& dotnet publish "$workDir\src\OpenFlux.Zen.Server.Cli\OpenFlux.Zen.Server.Cli.csproj" -c Release -o $InstallDir | Out-Null

# Copy native runtimes
$runtimesSrc = "$workDir\runtimes"
if (Test-Path $runtimesSrc) {
    $runtimesDst = "$InstallDir\runtimes"
...
$txtStep4 = if ($chosenLang -eq "ru") { "[4/7] Генерация ключей доступа и учётных данных..." } else { "[4/7] Generating security credentials and access keys..." }
Write-Host $txtStep4 -ForegroundColor Cyan

$credFile = "$InstallDir\data\.credentials"
$existingUser = ""
$existingPass = ""
$existingSecret = ""
...
# 7. Autostart Question & Task Scheduler Setup
$txtStep5 = if ($chosenLang -eq "ru") { "[5/7] Настройка автозапуска при загрузке системы..." } else { "[5/7] Configuring system autostart..." }
Write-Host $txtStep5 -ForegroundColor Cyan
...
& schtasks /delete /tn "OpenFluxZenServer" /f 2>$null | Out-Null
$taskAction = "\"$InstallDir\OpenFlux.Zen.Server.Web.exe\""
& schtasks /create /tn "OpenFluxZenServer" /tr $taskAction /sc onstart /ru SYSTEM /rl HIGHEST /f 2>$null | Out-Null
if ($LASTEXITCODE -ne 0) {
    & schtasks /create /tn "OpenFluxZenServer" /tr $taskAction /sc onlogon /rl HIGHEST /f 2>$null | Out-Null
}
if (-not $autostartEnabled) {
    & schtasks /change /tn "OpenFluxZenServer" /disable 2>$null | Out-Null
}

# 8. Register CLI in Machine PATH
$txtStep6 = if ($chosenLang -eq "ru") { "[6/7] Регистрация OpenFluxZenServer в системном PATH..." } else { "[6/7] Registering OpenFluxZenServer CLI command in PATH..." }
Write-Host $txtStep6 -ForegroundColor Cyan
...
$txtStep7 = if ($chosenLang -eq "ru") { "[7/7] Запуск OpenFlux Zen Server..." } else { "[7/7] Starting OpenFlux Zen Server..." }
Write-Host $txtStep7 -ForegroundColor Cyan

$psi = New-Object System.Diagnostics.ProcessStartInfo
$psi.FileName = "$InstallDir\OpenFlux.Zen.Server.Web.exe"
...
$title = if ($chosenLang -eq "ru") { "OpenFlux Zen Server — УСПЕШНО УСТАНОВЛЕН И ЗАПУЩЕН!" } else { "OpenFlux Zen Server — SUCCESSFULLY INSTALLED & STARTED!" }
